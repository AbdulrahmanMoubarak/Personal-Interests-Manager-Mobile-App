...
class FavGenresAdapter(var onClick: (itemId: String) -> Unit, var allSelected: Boolean = false) :
    RecyclerView.Adapter<FavGenresAdapter.FavGenresViewHolder>() {
    ...
    override fun onBindViewHolder(holder: FavGenresViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            if (!holder.selected) {
                ...
                val packageInfo =
                    MainApplication.getAppContext().getPackageManager().getPackageInfo(
                        MainApplication.getAppContext().getPackageName(),
                        PackageManager.GET_META_DATA
                    );
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
    ...
    class FavGenresViewHolder(itemView: View, allSelected: Boolean) :
        RecyclerView.ViewHolder(itemView) {
        val itemName: TextView
        var selected: Boolean = allSelected
        init {
